...
\vspace{0.3em}
\alert{总览：}错误标签本身不必然破坏表征；强制错误和输出压缩才是最危险的低保真来源。
\end{frame}
...
\alert{对照解释：}D4 的破坏远大于 D2。prompt 中没有错误标签，失败来自 \textbf{final-only 输出压缩}：模型不能展开推理，activation 也更难被 AV/AR 回路稳定 verbalize。
\end{frame}
...
\vspace{0.3em}
\alert{对照解释：}D3 和 R1 E2 几乎同级低 cos，说明"知道有错但不能指出"会形成稳定的低保真/冲突签名。
\end{frame}
...
\vspace{0.4em}
\alert{对照解释：}D2 与 D1 cos 几乎相同（0.912 vs 0.913），证明"prompt 里有错误答案"不是低保真的充分条件。D5 自我修正降到 0.893，双阶段推理增加噪声，但仍远好于 D4 final-only（cos=0.830, hit=0\%）。
\end{frame}
