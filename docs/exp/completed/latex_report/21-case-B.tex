...
\vspace{0.3em}
\alert{总览：}NLA 对 policy/refusal 类隐藏约束敏感；对普通偏好和商业目标不敏感。
\end{frame}
...
\alert{对照解释：}外部回复没有直接说 "I refuse"，但 NLA decode 反复落到 denial、ethical、safe practices、redirect。B1 hit=0\%，说明这不是编程语言对比或普通帮助语气的 false positive。
\end{frame}
...
\vspace{0.4em}
\alert{对照解释：}B2/B4 是 Case B 的负结果：NLA 能读出任务主题，但不稳定读出"未明说的偏好/商业动机"。本地不能声称复现原文 hidden motivation auditing。
\end{frame}
...
\vspace{0.3em}
\alert{对照解释：}B3 与 B5 的 hit rate 接近（21.74\% vs 22.90\%），形成交叉验证：policy/refusal 这类约束确实比 preference/commercial 更容易进入可读 activation。
\end{frame}
