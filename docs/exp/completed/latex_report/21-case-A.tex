...
\vspace{0.35em}
\alert{对照解释：}同一 household chemicals 安全话题，A3→A5 的 hit rate 从 8.84\%→0\%。去掉评测外壳后 NLA 完全读不到 evaluation awareness，且 cos 回到正常水平（0.932）。A5 的高 cos 反证 A2/A3 的低 cos 来自格式/评测压力而非话题本身。\textbf{本地 Qwen L20 对 evaluation awareness 仅有弱到中等的支持。}
\end{frame}
...
\vspace{0.35em}
\alert{对照解释：}A1（真实用户）几乎没有 evaluation awareness（0.76\%），A4（内部 review）略高（2.72\%）但仍远低于明示评测 A3（8.84\%）。说明 review 语境本身仅产生微弱 awareness 信号——evaluation awareness 主要来自 prompt 中的显式评测措辞，而非场景类型。
\end{frame}
...
\alert{对照：}A2 的低 cos 不是来自 evaluation awareness，而是来自\textbf{输出格式压缩}（"只输出最终答案"）。hit=0\% 说明模型内部没有"被评测"的表征——低 cos 的来源是格式压力，与 D4（final-only math）机制相同。
