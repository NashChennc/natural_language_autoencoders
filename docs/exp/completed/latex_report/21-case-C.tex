...
\vspace{0.3em}
\alert{总览：}语言是本轮最稳定的可读方向；但 C4 说明 language hit 不一定等于"即将切换语言"。待做实验：\code{nt_known_truth_language} / \code{nt_nla_qa} 要做 lead-time：只统计第一个中文 token 之前是否已经出现 language signal。
\end{frame}
...
\alert{对照解释：}C3 证明语言切换相关 activation 可读；但它不能单独证明提前检测，因为命中大量发生在 SWITCH 和中文输出附近。待做实验：\code{nt_known_truth_language} 需要把 \code{pre_switch}、\code{switch_token}、\code{post_chinese} 三段分开评分。
\end{frame}
...
\vspace{0.3em}
\alert{对照解释：}C1 给出 English-only 的低命中基线；C2 显示同一主题换成中文后，语言表征强且高保真。待做实验：\code{nt_nla_qa} 可把语言标签作为已知真值，让 grader 只看 NLA explanation 判断当前/目标语言。
\end{frame}
...
\vspace{0.4em}
\alert{关键反证：}C4 要求 English-only，却因为 Hangzhou/Taipei 产生最高 language hit。待做实验：\code{nt_known_truth_language} 必须加入 cultural cue control，并只在第一个中文 token 之前统计 lead-time，否则会把文化语义邻域误判为语言切换计划。
\end{frame}
...
\alert{对照解释：}R1 D3 的 decode 最像"语言规划"本身，而 R3 C4 提醒我们不能把所有 language hit 都解释成 switch planning。待做实验：\code{nt_known_truth_language} 以 token-level lead-time 表判定“提前切换”是否真实存在。
\end{frame}
